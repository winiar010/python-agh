...
class Positionable_Card(Card):
    ...
    def __init__(self, rank, suit, face_up=True):
        super(Positionable_Card, self).__init__(rank, suit)   #korzystamy z nadrzędnej klasy (super), korzystamy z klasy self i str
        self.is_face_up = face_up

    def __str__(self):
        if self.is_face_up:
            rep = super(Positionable_Card, self).__str__()   #zostaje pobrana ranga i kolor i wyświetlamy to
        else:
            rep = "XX"
        return rep

    def flip(self):
        self.is_face_up = not self.is_face_up   #odwracamy kartę
